             ...
             <div className="flex flex-col md:flex-row gap-6">
                 ...
                 <div className="md:w-48 flex-shrink-0">
                     ...
                     <ul className="space-y-1">
                         ...
                         {Array.from(new Set(communityNotes.map(h => h.category || '未分类'))).map(cat => (
                             <li key={cat}>
                                 <button
                 ...
                 <div className="flex-1">
                     <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                       {(communityFilterCategory ? communityNotes.filter(h => (h.category || '未分类') === communityFilterCategory) : communityNotes).map(item => (
               ...
               {(communityFilterCategory ? communityNotes.filter(h => (h.category || "未分类") === communityFilterCategory) : communityNotes).length === 0 && (
                   <div className="col-span-full py-16 text-center text-gray-500 bg-gray-50 rounded-xl border border-dashed border-gray-300">
          ...
          <div className={`grid gap-6 ${(result || preview || currentFileUrl) && viewMode === 'upload' ? 'lg:grid-cols-12' : viewMode === 'detail' ? 'lg:grid-cols-12 max-w-5xl mx-auto' : 'lg:grid-cols-1 max-w-3xl mx-auto'}`}>
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-3">
                            {customCategories.map(cat => (
                                <button
                                    ...
                                    className={`px-2 py-2 rounded-lg text-xs font-bold transition-all border break-words ${currentCategory === cat ? 'bg-purple-50/50 border-purple-600 text-purple-700 shadow-sm' : 'bg-white border-gray-200 text-gray-600 hover:border-purple-300 hover:bg-purple-50/20'}`}
                                >
                                    {cat}
                                </button>
                            ))}
            ...
            {result && (
                <div className={`flex flex-col gap-6 ${viewMode === 'detail' ? 'lg:col-span-12' : 'lg:col-span-8'}`}>
                    ...
                    <div className="flex justify-between items-center bg-gray-50/80 px-8 py-5 border-b border-gray-200">
                            ...
                            <div className="flex-1 w-full">
                                {isEditing ? (
                                  ...
                                  <div className="flex items-center space-x-2 mt-1">
                                      <span className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-xs font-bold rounded">{selectedStyle}风格</span>
                                      {isEditing ? (
                                          <select 
                                              value={currentCategory}
                                              onChange={(e) => setCurrentCategory(e.target.value)}
                                              className="px-2 py-0.5 bg-white border border-gray-300 text-gray-700 text-xs font-bold rounded focus:outline-none focus:border-purple-500"
                                          >
                                              {customCategories.map(cat => (
                                          ...
                                          <span className="px-2 py-0.5 bg-purple-50 text-purple-700 text-xs font-bold rounded">{currentCategory}</span>
                                      )}
                                  </div>
                            </div>
                        </div>
                    ...
                    <div className="p-8 overflow-y-auto max-h-[80vh] custom-scrollbar bg-white">
                        {isEditing ? (
                            <div className="flex flex-col h-full rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                                <MarkdownToolbar result={result} setResult={setResult} />
                                <textarea 
                                    id="markdown-editor"
                                    value={result} 
                                    onChange={e => setResult(e.target.value)}
                                    className="w-full flex-1 min-h-[500px] p-6 bg-slate-900 text-gray-50 focus:outline-none focus:ring-inset focus:ring-2 focus:ring-indigo-500 resize-y font-mono text-sm leading-relaxed"
                                    spellCheck="false"
                                />
                            </div>
                        ) : (
