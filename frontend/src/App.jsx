      ...
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl w-full max-w-md border border-gray-100 dark:border-slate-700">
          <div className="flex justify-center mb-8">
             <div className="bg-indigo-600 dark:bg-indigo-500 p-3 rounded-2xl shadow-lg shadow-indigo-200">
          ...
          <p className="text-center text-gray-500 dark:text-gray-400 dark:text-gray-400 mb-8 font-medium">{authMode === 'login' ? '欢迎回来，请登录' : '创建您的新账号'}</p>
          <form onSubmit={handleAuth} className="space-y-5">
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">用户名</label>
            ...
            {authMode === 'register' && (
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">联系方式 (邮箱/手机)</label>
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">密码</label>
            ...
            {authMode === 'login' && (
              <div className="flex justify-end mt-1">
                 <button type="button" onClick={() => alert("因未开启邮箱SMTP服务，暂时无法投递重置邮件。请联系系统管理员重置密码！")} className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">忘记密码？</button>

                        </div>
            )}
            <button type="submit" className="w-full bg-indigo-600 dark:bg-indigo-500 border border-transparent text-white rounded-xl py-3 px-4 font-bold text-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/30-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-md">
              {authMode === 'login' ? '登录' : '注册'}
            </button>
          </form>
          <div className="mt-6 text-center">
            <button type="button" onClick={(e) => { e.preventDefault(); setAuthMode(authMode === 'login' ? 'register' : 'login'); }} className="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300-800 transition-colors">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
          ...
          <div className="flex space-x-3 mt-4 md:mt-0 items-center">
              <button 
                ...
                className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-400 font-bold mr-4 flex items-center hover:text-indigo-700 dark:hover:text-indigo-300-600 dark:text-indigo-400 transition-colors px-2 py-1 rounded-md hover:bg-indigo-100 dark:hover:bg-indigo-900/30-50 dark:bg-indigo-900/20"
                title="个人主页"
              >
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-lg font-bold text-sm transition-all flex items-center ${viewMode === 'detail' && !currentNoteId ? 'bg-indigo-600 dark:bg-indigo-500 text-white shadow-md' : 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/30-100 dark:bg-indigo-900/50'}`}
            >
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-lg font-bold text-sm transition-all ${viewMode === 'upload' ? 'bg-gray-900 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:bg-gray-700'}`}
            >
              提取笔记
            </button>
            <button 
              onClick={handleGoToHistory}
              className={`px-4 py-2 rounded-lg font-bold text-sm transition-all flex items-center ${viewMode === 'history' || (viewMode === 'detail' && previousViewMode === 'history') ? 'bg-gray-900 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:bg-gray-700'}`}
            >
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-lg font-bold text-sm transition-all flex items-center ${viewMode === 'community' || (viewMode === 'detail' && previousViewMode === 'community') ? 'bg-purple-600 dark:bg-purple-500 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-purple-50 dark:bg-purple-900/20 hover:text-purple-600'}`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-bold text-sm transition-all flex items-center ${viewMode === 'profile' ? 'bg-blue-600 dark:bg-blue-500 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-blue-50 dark:bg-blue-900/20 hover:text-blue-600'}`}
            >
              主页
            </button>
            <ThemeToggle />
           ...
           <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-sm border border-gray-100 dark:border-slate-700">
             ...
             <div className="flex flex-col md:flex-row gap-6">
                 ...
                 <div className="md:w-48 flex-shrink-0">
                     <h3 className="text-sm font-bold text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase tracking-wider mb-4">按分类查看</h3>
                     <ul className="space-y-1">
                         <li>
                             <button
                                 onClick={() => setHistoryFilterCategory('')}
                                 className={`w-full text-left px-3 py-2 rounded-lg text-sm font-semibold transition-colors ${historyFilterCategory === '' ? 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:bg-gray-700'}`}
                             >
                                 全部笔记
                             </button>
                         </li>
                         {Array.from(new Set(history.map(h => h.category || '未分类'))).map(cat => (
                             <li key={cat}>
                                 <button
                                     ...
                                     className={`w-full text-left px-3 py-2 rounded-lg text-sm font-semibold transition-colors ${historyFilterCategory === cat ? 'bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:bg-gray-700'}`}
                                 >
                                     {cat}
                                 </button>
                             </li>
                    ...
                    <div className="flex justify-between items-start mb-3">
                          <p className="font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100 truncate pr-3 flex-1" title={item.title || item.filename}>{item.title || item.filename}</p>
                            <div className="flex flex-col gap-1 items-end">
                                <span className="text-[10px] bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 px-2 py-0.5 rounded font-semibold whitespace-nowrap border border-indigo-100 dark:border-indigo-900/30">{item.style}</span>
                                {item.category && item.category !== '未分类' && (
                                    <span className="text-[10px] bg-purple-50 dark:bg-purple-900/20 text-purple-700 px-2 py-0.5 rounded font-semibold whitespace-nowrap border border-purple-100">{item.category}</span>
                                )}
                            </div>
                    </div>
...
<div className="mt-4 text-sm text-indigo-600 dark:text-indigo-400 font-bold opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-between">
                 ...
                 {(historyFilterCategory ? history.filter(h => (h.category || "未分类") === historyFilterCategory) : history).length === 0 && (
                     <div className="col-span-full py-16 text-center text-gray-500 dark:text-gray-400 dark:text-gray-400 bg-gray-50 dark:bg-slate-900 rounded-xl border border-dashed border-gray-300 dark:border-slate-600">
                     ...
                     <ul className="space-y-1">
                         <li>
                             <button
                                 ...
                                 className={`w-full text-left px-3 py-2 rounded-lg text-sm font-semibold transition-colors ${communityFilterCategory === '' ? 'bg-purple-100 dark:bg-purple-900/50 text-purple-700' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:bg-gray-700'}`}
                             >
                                 全部笔记
                             </button>
                         </li>
                         {['AI', '矩阵分析', '机器学习', '编程开发', '系统架构', '科技前沿', '未分类'].map(cat => (
                             <li key={cat}>
                                 <button
                                     ...
                                     className={`w-full text-left px-3 py-2 rounded-lg text-sm font-semibold transition-colors ${communityFilterCategory === cat ? 'bg-purple-100 dark:bg-purple-900/50 text-purple-700' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:bg-gray-700'}`}
                                 >
                                     {cat}
                                 </button>
                             </li>
          ...
          <div className={`grid gap-6 ${(result || preview || currentFileUrl) && viewMode === 'upload' ? 'lg:grid-cols-12' : viewMode === 'detail' ? 'lg:grid-cols-12 max-w-5xl mx-auto' : 'lg:grid-cols-1 max-w-3xl mx-auto'}`}>
            ...
            <div className={`space-y-6 ${result || preview || currentFileUrl ? 'lg:col-span-4' : 'lg:col-span-1 border border-transparent'}`}>
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition">
                    ...
                    <div className="mb-5">
                        <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase tracking-wider mb-3">AI 提取风格</label>
                        <div className="grid grid-cols-2 gap-2">
                            {styles.map(style => (
                                <button
                                    key={style}
                                    onClick={() => setSelectedStyle(style)}
                                    className={`px-3 py-2.5 rounded-lg text-sm font-bold transition-all border ${selectedStyle === style ? 'bg-indigo-50 dark:bg-indigo-900/20/50 border-indigo-600 text-indigo-700 dark:text-indigo-400 shadow-sm' : 'bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-300 hover:border-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/30-50 dark:bg-indigo-900/20/20'}`}
                                >
                                    {style}
                                </button>
                            ))}
                    ...
                    <div className="mb-5">
                        <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase tracking-wider mb-3">笔记分类</label>
                        <div className="grid grid-cols-3 gap-2 mb-3">
                            {customCategories.map(cat => (
                                <div key={cat} className="relative group flex items-center justify-center w-full h-full">
                                    <button
                                        onClick={() => setCurrentCategory(cat)}
                                        className={`w-full h-full px-2 py-2 rounded-lg text-xs font-bold transition-all border break-words ${currentCategory === cat ? 'bg-purple-50 dark:bg-purple-900/20/50 border-purple-600 text-purple-700 shadow-[0_2px_8px_-2px_rgba(168,85,247,0.4)]' : 'bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-300 hover:border-purple-300 hover:bg-purple-50 dark:bg-purple-900/20/20'}`}
                                    >
                                        {cat}
                                    </button>
                                    <span 
                        ...
                        <div className="flex gap-2">
                            ...
                            <button 
                                ...
                                className="bg-purple-50 dark:bg-purple-900/20 text-purple-600 px-3 py-2 rounded-lg text-sm font-bold border border-purple-200 hover:bg-purple-100 dark:bg-purple-900/50 transition whitespace-nowrap"
                            >
                                添加
                            </button>
                        </div>
                    ...
                    <div className="relative border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-xl p-6 hover:border-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/30-50 dark:bg-indigo-900/20/30 focus-within:border-indigo-500 transition-all bg-gray-50 dark:bg-slate-900 text-center cursor-pointer group mb-5">
                        <input
                            type="file"
                            multiple
                        ...
                        <div className="flex flex-col items-center justify-center space-y-4 py-4">
                            <div className="p-3.5 bg-white dark:bg-slate-800 rounded-full shadow-sm group-hover:scale-110 group-hover:shadow-md transition-all">
                            ...
                            <div className="text-gray-700 dark:text-gray-200">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400 font-medium">支持 图片 / PDF / PPTX 合并处理</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {selectedFiles.length > 0 && (
                        <div className="mb-5 space-y-2 max-h-48 overflow-y-auto pr-1">
                            {selectedFiles.map((file, index) => (
                                <div key={index} className="flex justify-between items-center bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-3 shadow-sm hover:border-indigo-200 transition-colors">
                ...
                {(preview || currentFileUrl) && (
                    <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 relative group hidden lg:block">
                        <h2 className="text-xs font-bold mb-3 text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase tracking-wider">原始文档预览</h2>
                        <div className="bg-gray-50 dark:bg-slate-900/50 rounded-xl overflow-hidden flex items-center justify-center min-h-[250px] border border-gray-200 dark:border-slate-700">
                            {(preview || (currentFileUrl && currentFileUrl.match(/\.(jpeg|jpg|gif|png)$/i))) ? (
                                ...
                                <a href={currentFileUrl} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center p-8 text-indigo-600 dark:text-indigo-400 hover:bg-white dark:bg-slate-800 rounded-xl transition border border-transparent hover:border-indigo-100 dark:border-indigo-900/30 hover:shadow-sm">
                ...
                <div className={`flex flex-col gap-6 ${isFullscreen ? 'fixed inset-2 md:inset-6 z-[100] transition-all bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-0' : (viewMode === 'detail' ? 'lg:col-span-12' : 'lg:col-span-8')}`}>
                    ...
                    <div className="flex justify-between items-start bg-gray-50 dark:bg-slate-900/80 px-8 py-5 border-b border-gray-200 dark:border-slate-700">
                        <div className="flex items-start space-x-3 flex-1 min-w-0 mr-4">
                            {viewMode === 'detail' && (
                                  <button onClick={() => { setViewMode(previousViewMode); if(previousViewMode==='history') fetchHistory(searchQuery); else if(previousViewMode==='community') fetchCommunityNotes(searchQuery); }} className="p-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:bg-slate-500 rounded-lg text-gray-700 dark:text-gray-200 mr-2 transition-colors cursor-pointer" title="返回列表">
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" /></svg>
                                </button>
                            )}
                            <div className="p-2 bg-indigo-100 dark:bg-indigo-900/50 rounded-lg text-indigo-700 dark:text-indigo-400">
                            ...
                            <div className="flex-1 w-full">
                                {isEditing ? (
                                    <input 
                                        ...
                                        className="text-lg font-bold text-gray-900 dark:text-gray-100 dark:text-gray-100 m-0 w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1 focus:outline-none focus:border-indigo-500" 
                                    />
                                ) : (
                                  ...
                                  <div className="flex flex-wrap items-center gap-1.5 mt-2 w-full">
                                        <span className="px-2 py-1 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 text-xs font-bold rounded whitespace-nowrap self-start mt-0.5">{selectedStyle}风格</span>
                                        {isEditing ? (
                                            <>
                                                {customCategories.map(cat => (
                                                    <div 
                                                ...
                                                <input 
                                                    ...
                                                    className="w-28 text-xs px-3 py-1 text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-slate-900 border border-dashed border-gray-300 dark:border-slate-600 hover:border-purple-300 focus:border-purple-400 focus:bg-white dark:bg-slate-800 focus:outline-none rounded-full transition-all placeholder:text-[10px] placeholder:leading-tight"
                                                />
                                            </>
                                        ) : (
                                          <span className="px-3 py-1 bg-purple-50 dark:bg-purple-900/20 text-purple-700 text-xs font-bold rounded-full border border-purple-100 self-start mt-0.5">{currentCategory}</span>
                                      )}
                                  </div>
                            </div>
                        </div>
                        <div className="flex items-center space-x-2 shrink-0">
                            ...
                            {(isNoteOwner || !currentNoteId) && (
                                <button
                                    ...
                                    className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 text-indigo-700 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/30-100 dark:bg-indigo-900/50 px-4 py-2 rounded-lg font-bold transition-all text-sm flex items-center shadow-sm"
                                >
                                    {isEditing ? '保存' : '编辑'}
                                </button>
                            )}
                            <button
                                ...
                                className={`px-4 py-2 border rounded-lg font-bold transition-all text-sm flex items-center shadow-sm ${isFullscreen ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 border-amber-200 hover:bg-amber-100 dark:bg-amber-900/50' : 'bg-white dark:bg-slate-800 text-gray-700 dark:text-gray-200 border-gray-300 dark:border-slate-600 hover:bg-gray-50 dark:bg-slate-900'}`}
                            >
                                {isFullscreen ? (
                                    <>
                            ...
                            <button
                                ...
                                className="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:bg-slate-900 hover:text-gray-900 dark:text-gray-100 dark:text-gray-100 px-4 py-2 rounded-lg font-bold transition-all text-sm flex items-center shadow-sm hover:shadow"
                            >
                                <svg className="w-4 h-4 mr-2 text-gray-500 dark:text-gray-400 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                导出 MD
                            </button>
                            {currentNoteId && isNoteOwner && (
                                <button
                    ...
                    {/* Markdown Content */}
                    <div className={`p-8 overflow-y-auto custom-scrollbar bg-white dark:bg-slate-800 ${isFullscreen ? 'flex-1 h-full' : 'max-h-[80vh]'}`}>
                        {isEditing ? (
                            <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 w-full items-stretch ${isFullscreen ? 'h-full' : 'h-full min-h-[600px]'}`}>
                                ...
                                {/* Preview Side */}
                                <div className={`flex flex-col rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-slate-800 ${isFullscreen ? 'h-full' : 'h-[600px]'}`}>
                                    <div className="bg-gray-50 dark:bg-slate-900 px-4 py-3 border-b border-gray-200 dark:border-slate-700 flex items-center font-bold text-xs text-gray-500 dark:text-gray-400 dark:text-gray-400 uppercase tracking-wider">
                                    ...
                                    <div className="p-6 overflow-y-auto flex-1 prose prose-indigo max-w-none prose-headings:font-bold prose-h1:text-xl prose-h2:text-lg prose-a:text-indigo-600 dark:text-indigo-400 prose-img:rounded-xl dark:prose-invert bg-white dark:bg-slate-800 w-full">
                                        <ReactMarkdown 
                            ...
                            <div className="prose prose-indigo max-w-none prose-headings:font-bold prose-h1:text-2xl prose-h2:text-xl prose-a:text-indigo-600 dark:text-indigo-400 prose-img:rounded-xl dark:prose-invert">
                                <ReactMarkdown 
                                remarkPlugins={[remarkGfm, remarkMath]} 
                                rehypePlugins={[rehypeKatex]}
                                >
                ...
                {!isEditing && currentNoteId && (
                    <>
                        ...
                        {previousViewMode === 'community' && (
                            <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 mt-8">
